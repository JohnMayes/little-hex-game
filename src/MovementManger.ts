...
function calculateMovementPath(fromPos: Point, toPos: Point) {
  ...
  for (let i = 1; i <= distance; i++) {
    ...
    // const worldPos = grid.hexToPoint({ q: intermediateQ, r: intermediateR });
...
function selectUnit(unit: Unit) {
  ...
  console.log(`Selected unit at (${unit.pos.x}, ${unit.pos.y})`);
}

function attemptMove(unit: Unit, targetPos: Point) {
  // Don't allow movement if unit is already moving
  if (unit.isMoving) {
    console.log("Unit is already moving");
    return;
  }

  const targetHex = grid.pointToHex(targetPos);
  const isReachable = gameStore.state.reachableHexes?.some(hex => 
    hex.q === targetHex.q && hex.r === targetHex.r
  );
  ...
  if (isReachable) {
    ...
    if (movementCost <= unit.remainingMovement) {
      ...
      // console.log(path);
      console.log(pathToVecArray);
      unit.moveAnimated(pathToVecArray, movementCost, (remainingMovement: number) => {
        console.log(`Unit moved to (${targetPos.x}, ${targetPos.y}). Remaining: ${remainingMovement}`);
  ...
  update() {
    if (gameStore.state.phase === 'movement') {
      ...
      if (LittleJS.mouseWasPressed(0)) {
        ...
        const clickedUnit = gameStore.state.units.find(unit => 
      ...
      if (gameStore.state.selectedUnit && !gameStore.state.selectedUnit.isMoving) {
